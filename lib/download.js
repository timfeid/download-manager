...
var Download = function (url) {
  ...
  this.driver = null
  this.tempFile = helpers.filePath(config.tempDir, randomFileName())
  this.downloaded = 0
...
Download.prototype.createFilePath = function (headers) {
  return helpers.filePath(config.downloadDir,
    this.createFileName(headers))
}

Download.prototype.headers = function (response) {
  var Driver = this.findDriver(response)
  this.file = this.createFilePath(response.headers)
