...
Part.prototype.start = function () {
  this.request()
    .then(res => {
      ...
      res.data.on('error', this.download.error.bind(this.download))
      res.data.pipe(fs.createWriteStream(this.file))
    })
    .catch(err => this.download.error.bind(this.download, err))
}
...
Part.prototype.request = function () {
  const opts = {
    method: 'get',
    responseType: 'stream',
    onDownloadProgress: this.progress.bind(this),
